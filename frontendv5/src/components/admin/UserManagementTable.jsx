...
const UserManagementTable = ({ users, onRefresh, onSort, renderSortIcon }) => {
  const { executeApi, isLoading } = useApiWithStore('userManagement');
  ...
  const handleDeleteUser = useCallback(
    async (userId, userName, userRole) => {
      if (userRole === USER_ROLES.ADMIN) {
        // Sử dụng showNotification thay vì toast
        executeApi(
          () =>
            Promise.reject(new Error('Không thể vô hiệu hóa tài khoản Admin.')),
          { showError: true }
        );
        return;
      }        const confirmed = safeConfirm(
          `Bạn có chắc chắn muốn vô hiệu hóa người dùng "${userName}" (ID: ${userId}) không? ` +
            `Hành động này sẽ chuyển trạng thái của họ thành "Suspended".`
        );

      if (!confirmed) return;

      await executeApi(() => userService.softDeleteUserByAdmin(userId), {
    ...
    [executeApi, onRefresh]
  );

  /**
   * Get CSS classes cho role badge
   * @param {string} roleName - Tên role
   * @returns {string} CSS classes
   */
  const getRoleSpecificClasses = useCallback((roleName) => {
    switch (roleName) {
   ...
   * @returns {string} CSS classes
   */
  const getStatusSpecificClasses = useCallback((status) => {
    switch (status) {
          ...
          {users.map(user => (
            <tr key={user.id} className='hover:bg-gray-50 transition-colors'>
              ...
              <td className='px-6 py-4 whitespace-nowrap text-sm'>
                <span
                  className={`px-2.5 py-0.5 inline-flex text-xs leading-5 font-semibold rounded-full ${getRoleSpecificClasses(user.role)}`}
                >
                  {user.role}
                </span>
              </td>
              <td className='px-6 py-4 whitespace-nowrap text-sm text-center'>
                <span
                  className={`px-2.5 py-0.5 inline-flex text-xs leading-5 font-semibold rounded-full ${getStatusSpecificClasses(user.status)}`}
                >
                  {user.status}
                </span>
              </td>
              ...
              <td className='px-6 py-4 whitespace-nowrap text-sm font-medium text-center space-x-1'>
                ...
                {user.role !== USER_ROLES.ADMIN &&
                  user.status === USER_STATUSES.ACTIVE && (
                    <Button
                      onClick={() =>
                        handleDeleteUser(user.id, user.fullName, user.role)
                      }
                      variant='icon'
