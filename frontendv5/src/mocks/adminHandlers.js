...
const mockUsers = Array.from({ length: 25 }, (_, i) => {
  ...
  return {
    ...
    role:
      id === 1
        ...
        : id === 2
          ...
          : id % 5 === 0
            ? 'Staff'
            : 'Member', // Đảm bảo có user Staff cố định để test
    status: id % 4 === 0 ? 'Suspended' : id % 3 === 0 ? 'Pending' : 'Active',
    createdAt: new Date(
      Date.now() - (30 - id) * 1000 * 60 * 60 * 24
    ).toISOString(),
    ...
    lastDonationDate:
      id % 4 === 0 ? `2024-0${(id % 12) + 1}-0${(id % 28) + 1}` : null,
...
const mockDonationRequests = Array.from({ length: 15 }, (_, i) => ({
  id: i + 1,
  ...
  requestDate: new Date(Date.now() - (15 - i) * 1000 * 60 * 60 * 24)
    .toISOString()
    .split('T')[0],
  preferredDate: new Date(Date.now() + i * 1000 * 60 * 60 * 24)
    .toISOString()
    .split('T')[0],
...
const mockBloodInventory = [
  {
    id: 1,
    bloodTypeId: mockBloodTypes[0].id,
    quantityMl: 10000,
    lastUpdated: new Date().toISOString(),
  },
  {
    id: 2,
    bloodTypeId: mockBloodTypes[1].id,
    quantityMl: 8000,
    lastUpdated: new Date().toISOString(),
  },
  {
    id: 3,
    bloodTypeId: mockBloodTypes[2].id,
    quantityMl: 5000,
    lastUpdated: new Date().toISOString(),
  },
];
...
export const adminHandlers = [
  ...
  http.put(`${API_URL}/admin/users/:userId`, async ({ request, params }) => {
    ...
    if (
      updatedData.email &&
      mockUsers.some(
        (u, idx) => idx !== userIndex && u.email === updatedData.email
      )
    ) {
      return HttpResponse.json({ email: 'Email đã tồn tại.' }, { status: 409 });
    }
    if (
      ...
      mockUsers.some(
        (u, idx) => idx !== userIndex && u.username === updatedData.username
      )
    ) {
      return HttpResponse.json(
        { username: 'Tên đăng nhập đã tồn tại.' },
    ...
    if (
      updatedData.bloodTypeId !== undefined &&
      updatedData.bloodTypeId !== null
    ) {
  ...
  http.get(`${API_URL}/blood-types`, ({ request }) => {
    ...
    const filtered = mockBloodTypes.filter(bt => {
      ...
      const matchesComponent =
        !componentType || bt.componentType === componentType;
  ...
  http.post(`${API_URL}/blood-types`, async ({ request }) => {
    ...
    if (
      mockBloodTypes.some(
        bt =>
          bt.bloodGroup === newTypeReq.bloodGroup &&
          bt.componentType === newTypeReq.componentType
      )
    ) {
      return HttpResponse.json(
        { message: 'Loại máu và thành phần này đã tồn tại.' },
        { status: 409 }
      );
    }
    const newType = {
  ...
  http.put(`${API_URL}/blood-types/:id`, async ({ request, params }) => {
    const updatedData = await request.json();
    const typeIndex = mockBloodTypes.findIndex(
      bt => bt.id === parseInt(params.id)
    );
    if (typeIndex === -1)
      return HttpResponse.json(
    ...
    mockBloodTypes[typeIndex] = {
      ...
      description:
        updatedData.description !== undefined
          ...
          : mockBloodTypes[typeIndex].description,
      updatedAt: new Date().toISOString(),
    };
  ...
  http.delete(`${API_URL}/blood-types/:id`, ({ params }) => {
    ...
    mockCompatibilityRules = mockCompatibilityRules.filter(
      r =>
        r.donorBloodType.id !== idToDelete &&
        r.recipientBloodType.id !== idToDelete
    );
  ...
  http.get(`${API_URL}/blood-compatibility`, ({ request }) => {
    ...
    const filtered = mockCompatibilityRules.filter(rule => {
      const matchesSearch =
        !search ||
        rule.id.toString().includes(search) ||
        (rule.notes && rule.notes.toLowerCase().includes(search));
      const matchesCompatible =
        isCompatibleParam === null ||
        isCompatibleParam === '' ||
        rule.isCompatible === (isCompatibleParam === 'true');
      const matchesEmergency =
        isEmergencyParam === null ||
        isEmergencyParam === '' ||
        rule.isEmergencyCompatible === (isEmergencyParam === 'true');
  ...
  http.post(`${API_URL}/blood-compatibility`, async ({ request }) => {
    ...
    const donorBloodType = mockBloodTypes.find(
      bt => bt.id === data.donorBloodTypeId
    );
    const recipientBloodType = mockBloodTypes.find(
      bt => bt.id === data.recipientBloodTypeId
    );
    ...
    const exists = mockCompatibilityRules.some(
      rule =>
    ...
    if (exists) {
      return HttpResponse.json(
        { message: 'Quy tắc tương thích này đã tồn tại.' },
        { status: 409 }
      );
    }
  ...
  http.put(
    `${API_URL}/blood-compatibility/:id`,
    async ({ request, params }) => {
      const data = await request.json();
      const ruleIndex = mockCompatibilityRules.findIndex(
      ...
      if (exists) {
        return HttpResponse.json(
          {
            message:
              'Quy tắc tương thích này đã tồn tại cho cặp loại máu khác.',
          },
          { status: 409 }
        );
      }
  ...
  http.get(`${API_URL}/donation-requests/:id`, ({ params }) => {
    const request = mockDonationRequests.find(
      req => req.id === parseInt(params.id)
    );
    if (!request)
      return HttpResponse.json(
  ...
  http.post(`${API_URL}/donation-requests`, async ({ request }) => {
    ...
    if (!donor || !bloodType)
      return HttpResponse.json(
        { message: 'Invalid donor or blood type ID' },
        { status: 400 }
      );
  ...
  http.put(`${API_URL}/donation-requests/:id`, async ({ request, params }) => {
    const updatedData = await request.json();
    const reqIndex = mockDonationRequests.findIndex(
      req => req.id === parseInt(params.id)
    );
    if (reqIndex === -1)
      return HttpResponse.json(
    ...
    let bloodTypeDescription =
      mockDonationRequests[reqIndex].bloodTypeDescription;
    if (
      updatedData.bloodTypeId !== undefined &&
      updatedData.bloodTypeId !== null
    ) {
  ...
  // == BLOOD INVENTORY (NEW) ==
  http.get(`${API_URL}/blood-inventory`, ({ request: _request }) => {
  ...
  http.post(`${API_URL}/blood-inventory`, async ({ request }) => {
    ...
    const existingInventory = mockBloodInventory.find(
      item => item.bloodTypeId === newItem.bloodTypeId
    );
    if (existingInventory) {
      return HttpResponse.json(
        { message: 'Tồn kho cho loại máu này đã tồn tại, vui lòng cập nhật.' },
        { status: 409 }
      );
    }
  ...
  http.put(`${API_URL}/blood-inventory/:id`, async ({ request, params }) => {
    const updatedData = await request.json();
    const itemIndex = mockBloodInventory.findIndex(
    ...
    if (updatedData.bloodTypeId) {
      const bloodType = mockBloodTypes.find(
        bt => bt.id === updatedData.bloodTypeId
      );
      if (!bloodType)
        return HttpResponse.json(
          { message: 'Blood type not found for update' },
          { status: 400 }
        );
    }
  ...
  http.post(`${API_URL}/emergency-requests`, async ({ request }) => {
    ...
    console.warn('MSW: New emergency request received:', newEmergencyReq);
    return HttpResponse.json(
