...
import { REQUEST_STATUS, URGENCY_LEVELS, STATUS_COLORS } from '../../utils/constants';
...
const AdminBloodRequestsPage = () => {
  ...
  const handleSubmitForm = async (formData) => {
    try {
      ...
      toast.error(modalMode === 'create' ? 'Lỗi khi tạo yêu cầu' : 'Lỗi khi cập nhật yêu cầu');
    } finally {
  ...
  const columns = [
    ...
    },
    {
      key: 'bloodType',
      title: 'Nhóm máu',
      render: (row) => row.bloodType?.name || 'N/A',
    },
    {
      key: 'quantityInUnits',
    ...
    },
    {
      ...
      render: (row) => (
        <span className={`px-2 py-1 rounded-full text-xs font-medium ${
          ...
          row.urgency === 'URGENT' ? 'bg-orange-100 text-orange-800' :
  ...
  return (
    <AdminPageLayout>      <AdminContentWrapper
      ...
      >
        ...
        <Modal
          ...
          title={
            modalMode === 'create' ? 'Tạo yêu cầu máu mới' :
            modalMode === 'edit' ? 'Chỉnh sửa yêu cầu máu' :
            ...
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <p className="mt-1 text-sm text-gray-900">{selectedRequest?.bloodType?.name}</p>
                </div>
                <div>
                  <label className="block text-sm font-medium text-gray-700">Số lượng</label>
                  <p className="mt-1 text-sm text-gray-900">{selectedRequest?.quantityInUnits} đơn vị</p>
                </div>
                <div>
                  <label className="block text-sm font-medium text-gray-700">Mức độ khẩn cấp</label>
                  <p className="mt-1 text-sm text-gray-900">{URGENCY_LEVELS[selectedRequest?.urgency]}</p>
                </div>
                <div>
                  <label className="block text-sm font-medium text-gray-700">Trạng thái</label>
                  <p className="mt-1 text-sm text-gray-900">{REQUEST_STATUS[selectedRequest?.status]}</p>
                </div>
