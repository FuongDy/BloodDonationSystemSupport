    ...
    const fetchBloodData = useCallback(async () => {
        ...
        try {
            const [typesData, rulesData] = await Promise.all([
                bloodTypeService.getAll(0, 500),
                bloodCompatibilityService.getAll(0, 1000)
            ]);

            const allTypes = typesData.content || [];
            setAllBloodTypes(allTypes);
            setCompatibilityRules(rulesData.content || []);

            // SỬA: Lọc dữ liệu bằng thuật ngữ tiếng Việt "Toàn phần"
            const wholeTypes = allTypes.filter(bt => bt.componentType === 'Toàn phần');
            setWholeBloodTypes(wholeTypes);

            const uniqueGroups = [...new Set(allTypes.map(t => t.bloodGroup))];
            // SỬA: Lọc dữ liệu bằng thuật ngữ tiếng Việt "Toàn phần"
            const uniqueComponents = [...new Set(allTypes.map(t => t.componentType))].filter(c => c !== 'Toàn phần');
                ...
                <div className="p-4 bg-red-50 rounded-lg shadow-inner">
                    ...
                    {activeSubTab === 'whole' ? (
                        <div>
                            ...
                            >
                                {wholeBloodTypes.map(bt => <option key={bt.id} value={bt.id}>{bt.description}</option>)}
                            </select>
                        </div>
                    ) : (
                        <div>
