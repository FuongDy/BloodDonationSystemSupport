...
const ProfileSecurityPage = () => (
  <div className='bg-white shadow-xl rounded-lg p-6 md:p-8'>
    ...
    <p>Nội dung trang bảo mật...</p>
  </div>
);
const ProfileDonationSchedulePage = () => (
  <div className='bg-white shadow-xl rounded-lg p-6 md:p-8'>
    <h2>Lịch hiến máu</h2>
    <p>Nội dung trang lịch hiến máu...</p>
  </div>
);
const ProfileDonationHistoryPage = () => (
  <div className='bg-white shadow-xl rounded-lg p-6 md:p-8'>
    <h2>Lịch sử hiến máu</h2>
    <p>Nội dung trang lịch sử hiến máu...</p>
  </div>
);
const ProfileNotificationsPage = () => (
  <div className='bg-white shadow-xl rounded-lg p-6 md:p-8'>
    <h2>Cài đặt thông báo</h2>
    <p>Nội dung trang cài đặt thông báo...</p>
  </div>
);
const ProfileSettingsPage = () => (
  <div className='bg-white shadow-xl rounded-lg p-6 md:p-8'>
    <h2>Cài đặt tài khoản</h2>
    <p>Nội dung trang cài đặt tài khoản...</p>
  </div>
);
      ...
      {isEditing ? (
        ...
        <div className='space-y-4 mt-6'>
          ...
          <InfoRow
            icon={AlertTriangle}
            label='Liên hệ khẩn cấp'
            value={profileData.emergencyContact || 'Chưa cập nhật'}
          />
          <InfoRow
            icon={Droplet}
            label='Nhóm máu'
            value={profileData.bloodTypeDescription || 'Chưa cập nhật'}
          />
          <InfoRow
            icon={ShieldCheck}
            label='Bệnh lý'
            value={profileData.medicalConditions || 'Không có'}
          />
          <InfoRow
            icon={CalendarDays}
            label='Hiến máu lần cuối'
            value={
          ...
          <InfoRow
            ...
            valueColor={
              profileData.isReadyToDonate ? 'text-green-600' : 'text-red-600'
            }
          />
        </div>
      )}
...
const UserProfilePage = () => {
  ...
  const fetchProfile = useCallback(async () => {
    ...
    try {
      ...
      setError(
        err.message || 'Không thể tải thông tin hồ sơ. Vui lòng thử lại.'
      );
      setProfileData(null);
  ...
  const fetchBloodTypes = useCallback(async () => {
    try {
      const types = await bloodTypeService.getAll();
      setBloodTypes(types);
    } catch (err) {
  ...
  useEffect(() => {
    ...
    if (isAuthenticated && user) {
      ...
      setError(
        'Đã xác thực nhưng thông tin người dùng không đầy đủ hoặc bị lỗi. Vui lòng thử đăng nhập lại.'
      );
      setLoading(false);
      setProfileData(null);
    } else {
  ...
  if (loading) {
    // Page specific data loading (auth is done)
    return (
      <div className='flex flex-col min-h-screen bg-gray-50'>
            ...
            <div className='flex-1 min-w-0 order-1 lg:order-2'>
              <div className='bg-white shadow-xl rounded-lg p-6 md:p-8 flex flex-col justify-center items-center min-h-96'>
                <div className='text-center'>
                  ...
                  {isAuthenticated &&
                    user &&
                    !error.includes('thông tin người dùng không đầy đủ') && (
                  ...
                  {isAuthenticated && !user && (
                    ...
                    >
                      <Button variant='primary'>Đăng nhập lại</Button>
                    </Link>
                  )}
                  {!isAuthenticated &&
                    !error.includes('thông tin người dùng không đầy đủ') && (
  ...
  if (!profileData && !isAuthenticated) {
    // Case where user is not logged in and no specific error was set for that
    return (
      <div className='flex flex-col min-h-screen bg-gray-50'>
        <main className='flex-grow container mx-auto px-4 py-8'>
          <div className='flex flex-col lg:flex-row gap-6 max-w-7xl mx-auto'>
  ...
  if (!profileData && isAuthenticated) {
    // Logged in, but no profile data (and no error reported by fetch)
    return (
      <div className='flex flex-col min-h-screen bg-gray-50'>
        <main className='flex-grow container mx-auto px-4 py-8'>
          <div className='flex flex-col lg:flex-row gap-6 max-w-7xl mx-auto'>
              ...
              <div className='bg-white shadow-xl rounded-lg p-6 md:p-8 flex flex-col justify-center items-center min-h-96'>
                <div className='text-center'>
                  ...
                  <p className='text-gray-600'>
                    Không tìm thấy thông tin hồ sơ cho tài khoản này.
                  </p>
          ...
          <div className='flex-1 min-w-0 order-1 lg:order-2'>
            <Routes>
              ...
              <Route
                ...
                element={<ProfileDonationHistoryPage />}
              />
              <Route
                path='notifications'
...
const InfoRow = ({
  ...
  <div className='py-3 sm:grid sm:grid-cols-3 sm:gap-4 border-b border-gray-200 last:border-b-0'>
    ...
    <dd className={`mt-1 text-sm ${valueColor} sm:mt-0 sm:col-span-2`}>
      {value}
    </dd>
  </div>
);
