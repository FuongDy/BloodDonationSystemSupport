...
const UserManagementTable = ({ users, onRefresh, onSort, renderSortIcon }) => {
  ...
  const handleDeleteUser = async (userId, userName, userRole) => {
    ...
    if (
      safeConfirm(
        `Bạn có chắc chắn muốn vô hiệu hóa người dùng "${userName}" (ID: ${userId}) không? Hành động này sẽ chuyển trạng thái của họ thành "Suspended".`
      )
    ) {
      const toastId = toast.loading(
        `Đang vô hiệu hóa người dùng ${userName}...`
      );
      try {
        await userService.softDeleteUserByAdmin(userId);
        toast.success(`Người dùng ${userName} đã được vô hiệu hóa.`, {
          id: toastId,
        });
        if (onRefresh) onRefresh();
      } catch (error) {
        toast.error(`Lỗi khi vô hiệu hóa người dùng: ${error.message}`, {
          id: toastId,
        });
      }
    }
