...
const AdminBloodCompatibilityPage = () => {
  ...
  const handleDelete = async id => {
    ...
    if (
      window.confirm(
        `Bạn có chắc chắn muốn xóa quy tắc tương thích (ID: ${id}) không?`
      )
    ) {
    ...
    <div className='p-6'>
      ...
      {isLoading && !rulesPage ? (
          ...
          {rulesPage.totalPages > 1 && (
            <div className='mt-6 flex justify-between items-center'>
              <div className='text-sm text-gray-700'>
                Trang{' '}
                <span className='font-medium'>{rulesPage.number + 1}</span> /{' '}
                <span className='font-medium'>{rulesPage.totalPages}</span> (
                {rulesPage.totalElements} quy tắc)
              </div>
              <Pagination
        ...
        <p className='text-center text-gray-500 py-8'>
          Chưa có quy tắc tương thích nào.
        </p>
      )}
      {isModalOpen && (
