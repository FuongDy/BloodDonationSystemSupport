...
import { getRoleSpecificClasses, getStatusSpecificClasses } from '../../utils/formatters';
      ...
      <table className='min-w-full divide-y divide-gray-200'>
        <thead className='bg-gray-50'>
          ...
          {users.map(user => {
            ...
            if (user.role !== USER_ROLES.ADMIN && user.status === USER_STATUSES.ACTIVE) {
              userActions.push({
                label: 'Vô hiệu hóa',
                icon: Trash2,
                variant: 'icon',
                className: 'text-red-600 hover:text-red-800',
                onClick: () => _handleDeleteUser(user.id, user.fullName, user.role),
                disabled: isLoading
              });
            } else if (user.role === USER_ROLES.ADMIN) {
              userActions.push({
                label: 'Không thể vô hiệu hóa Admin',
                icon: ShieldQuestion,
                variant: 'icon',
