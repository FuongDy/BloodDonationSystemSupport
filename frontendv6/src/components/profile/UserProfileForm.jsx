...
const UserProfileForm = ({ mode = "view" }) => {
  ...
  const fetchUserData = useCallback(async (forceRefresh = false) => {
    ...
    try {
      ...
      console.error(
      error);      toast.error(`Cập nhật thất bại: ${error.message}`);
  ...
  const handleConfirmSubmit = async () => {
    ...
    try {
      const _updatedUser = await userService.updateProfile(requestData);
      toast.success('Cập nhật thông tin thành công!', { id: toastId });
      setUser(null);
      await fetchUserData(true);
      navigate('/profile');    } catch (error) {
      toast.error(`Cập nhật thất bại: ${error.message || 'Vui lòng thử lại.'}`, { id: toastId });
          ...
          <form onSubmit={handleSubmit} className='p-6 space-y-6'>
            ...
            <div className='grid grid-cols-1 md:grid-cols-2 gap-6'>
              ...
              <InputField
                ...
                disabled={isSubmitting}
              />
            </div>            <div className='grid grid-cols-1 md:grid-cols-2 gap-6'>              <DatePicker
                label='Ngày sinh'
                name='dateOfBirth'
      ...
      <Modal
        ...
        footerContent={
          <>
            ...
            <Button variant="primary" onClick={handleConfirmSubmit} loading={isSubmitting}>
              Xác nhận
            </Button>
          </>
        }
