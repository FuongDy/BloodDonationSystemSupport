...
const AdminUserDetailPage = () => {
  ...
  const fetchUserData = useCallback(async () => {
    ...
    try {
      if (process.env.NODE_ENV === 'development') {
        console.warn(`Fetching user data for ID (component): ${userId}`);
      }
      const userData = await userService.getUserByIdForAdmin(userId);
      if (process.env.NODE_ENV === 'development') {
        console.warn('User data received (component):', userData);
      }
      setUser(userData);
    } catch (error) {
  ...
  if (!user && !isLoading) {
    // Nếu không tải nữa VÀ user là null (có thể do lỗi fetch hoặc user không tồn tại)
    return (
      <div className='p-6 text-center'>
        <p className='text-xl text-gray-700'>
          Không tìm thấy thông tin người dùng hoặc có lỗi xảy ra.
        </p>
        <Link to='/admin/users'>
