...
let mockCompatibilities = [
  ...
  },
  {
    id: 3,
    donorBloodType: mockBloodTypes[0],
    recipientBloodType: mockBloodTypes[2],
    isCompatible: true,
  },
  {
    id: 4,
    donorBloodType: mockBloodTypes[1],
    recipientBloodType: mockBloodTypes[3],
    isCompatible: true,
  },
  {
    id: 5,
    donorBloodType: mockBloodTypes[2],
    recipientBloodType: mockBloodTypes[6],
    isCompatible: false,
  },
...
export const bloodCompatibilityHandlers = [
  ...
  http.post(`${API_URL}/api/blood-compatibility`, async ({ request }) => {
    ...
    const recipient = mockBloodTypes.find(
      bt => bt.id === newRuleData.recipientId
    );

    if (!donor || !recipient) {
      return new HttpResponse(
        JSON.stringify({ message: 'Donor or Recipient not found' }),
        { status: 400 }
      );
    }
    ...
    const newId =
      mockCompatibilities.length > 0
        ? Math.max(...mockCompatibilities.map(c => c.id)) + 1
        : 1;
    const newRule = {
      id: newId,
  ...
  http.put(
    `${API_URL}/api/blood-compatibility/:id`,
    async ({ params, request }) => {
      const { id } = params;
      const updates = await request.json();
      const ruleIndex = mockCompatibilities.findIndex(
      ...
      if (updates.donorId) {
        updatedRule.donorBloodType = mockBloodTypes.find(
          bt => bt.id === updates.donorId
        );
      }
      if (updates.recipientId) {
        updatedRule.recipientBloodType = mockBloodTypes.find(
          bt => bt.id === updates.recipientId
        );
      }
      ...
      console.log(
        `[MSW] Handled PUT /api/blood-compatibility/${id}`,
        updatedRule
      );
  ...
  http.delete(`${API_URL}/api/blood-compatibility/:id`, ({ params }) => {
    ...
    mockCompatibilities = mockCompatibilities.filter(
      c => c.id !== parseInt(id, 10)
    );
