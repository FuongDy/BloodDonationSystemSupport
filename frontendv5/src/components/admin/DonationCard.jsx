  ...
  className = ''
    ...
    return actions.map(action => {
      ...
      return {
        ...
        label: (
          <>
            {IconComponent && <IconComponentComponent className='w-4 h-4 mr-1' />}
            {action.label}
          </>
        )
      };
    ...
    >      <CardHeader className='pb-3'>
      ...
      <CardContent className='space-y-3'>
        ...
        {donation.appointment?.scheduledDate && (
          <InfoRow 
            label="Ngày hẹn" 
            value={<DateTimeDisplay date={donation.appointment.scheduledDate} format="date" />} 
          />
        )}
        ...
        <div className='flex items-center space-x-2 pt-2'>
          ...
          <span className='text-sm text-gray-500'>
            Cập nhật: <DateTimeDisplay date={donation.updatedAt} format="date" />
          </span>
        </div>
