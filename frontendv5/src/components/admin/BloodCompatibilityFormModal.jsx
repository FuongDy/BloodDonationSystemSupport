...
const BloodCompatibilityFormModal = ({
  ...
  const validateForm = () => {
    ...
    if (!formData.recipientBloodTypeId)
      newErrors.recipientBloodTypeId = 'Vui lòng chọn loại máu người nhận.';
    if (
      formData.compatibilityScore === '' ||
      isNaN(formData.compatibilityScore)
    ) {
      ...
      {isDataLoading ? (
        ...
        >
          ...
          <div className='flex items-center space-x-8 pt-2'>
            ...
            <div className='flex items-center'>
              <input
                id='isEmergencyCompatible'
                name='isEmergencyCompatible'
                type='checkbox'
                checked={formData.isEmergencyCompatible}
                onChange={handleChange}
                disabled={isLoading || isReadOnly}
                className='h-4 w-4 text-red-600 focus:ring-red-500 border-gray-300 rounded'
              />
              <label
                htmlFor='isEmergencyCompatible'
                className='ml-2 block text-sm text-gray-900'
              >
                Tương thích khẩn cấp
              </label>
            </div>
          </div>
          <InputField
