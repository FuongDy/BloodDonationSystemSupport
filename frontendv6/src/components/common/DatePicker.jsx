...
const DatePicker = ({ 
  ...
  useEffect(() => {
    const handleClickOutside = (event) => {
      if (showCalendar && datePickerRef.current && !datePickerRef.current.contains(event.target)) {
  ...
  useEffect(() => {
    if (selectedDate !== value && onChange && name) {
      onChange({ target: { value: selectedDate, name: name } });
    }
    ...
    for (let i = 0; i < 42; i++) {
      ...
      if (selectedDate) {
        if (/^\d{2}-\d{2}-\d{4}$/.test(selectedDate)) {
          ...
          isSelected = selectedDateObj.setHours(0, 0, 0, 0) === date.setHours(0, 0, 0, 0);
        } else if (selectedDate.includes('T')) {
          isSelected = selectedDate.split('T')[0] === date.toISOString().split('T')[0];
        } else {
  ...
  const handleDateClick = (day) => {
    ...
    const date = day.date;
    const day_part = String(date.getDate()).padStart(2, '0');
    const month_part = String(date.getMonth() + 1).padStart(2, '0');
    const year_part = date.getFullYear();
    const formattedDate = `${day_part}-${month_part}-${year_part}`;
      ...
      <div className="relative date-picker-container">
        ...
        {showCalendar && (
          <div className="absolute top-full left-0 mt-2 bg-white border border-gray-200 rounded-lg shadow-lg z-50 p-4 w-80">            {/* Month/Year Navigation */}
            ...
            {/* Year Picker */}
            {showYearPicker && (              <div className="max-h-60 overflow-y-auto mb-4">
