...
import { LayoutDashboard, Users, LogOut, ShieldAlert, Home as HomeIcon, Droplets, GitCompareArrows, Menu, ChevronsLeft, ChevronsRight } from 'lucide-react';
import { useAuth } from '../../hooks/useAuth';

// Receive isOpen and toggleSidebar as props
    ...
    return (
        ...
        >
            <div className="flex items-center justify-between mb-6">
                {isOpen && <h2 className="text-xl font-semibold text-slate-50">Admin Panel</h2>}
                {/* Use toggleSidebar from props */}
                <button onClick={toggleSidebar} className="text-slate-300 hover:text-white p-1 rounded-md focus:outline-none focus:ring-2 focus:ring-slate-500">
                ...
                <ul>
                    {menuItems.map((item) => (
                        <li key={item.path} className="mb-1" title={!isOpen ? item.label : ''}>
                            ...
                            >
                                <item.icon size={20} className={`${isOpen ? 'mr-3' : 'mr-0'} group-hover:text-red-400 ${isActive(item.path) ? 'text-white' : 'text-slate-400'}`} />
                    ...
                    <li title={!isOpen ? "Về trang chủ" : ''}>
                        ...
                        >
                            <HomeIcon size={20} className={`${isOpen ? 'mr-3' : 'mr-0'} text-slate-400 group-hover:text-red-400`} />
                    ...
                    <li className="mt-1" title={!isOpen ? "Đăng xuất" : ''}>
                        ...
                        >
                            <LogOut size={20} className={`${isOpen ? 'mr-3' : 'mr-0'} text-slate-400 group-hover:text-red-400`} />
