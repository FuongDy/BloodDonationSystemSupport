...
const AdminAppointmentManagementPage = () => {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const processesNeedingAppointment = allProcesses.filter(p => 
  ...
  const handleRequestReschedule = async () => {
    ...
    try {
      await appointmentService.requestReschedule(selectedAppointment.appointment.id, rescheduleForm);
  ...
  const appointmentColumns = [
      ...
      render: (value, process) => value ? (
        <div className="space-y-1">
          <div className="flex items-center space-x-1">
            <Calendar size={14} className={isUpcoming(value.appointmentDateTime) ? 'text-blue-500' : 'text-gray-400'} />
            <span className={`text-sm ${isUpcoming(value.appointmentDateTime) ? 'font-medium' : 'text-gray-500'}`}>
    ...
    },
    {
      key: 'status',
      title: 'Trạng thái',
      render: (value) => <StatusBadge status={value} variant={getStatusColor(value)} />    },
    {
    ...
    },
    {
      ...
      render: (_, process) => (
        <div className="flex space-x-2">
          {process.appointment && isUpcoming(process.appointment.appointmentDateTime) && (
            <Button
              size="sm"
              variant="outline"
              onClick={() => {
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold text-green-700">
                  {appointments.filter(a => a.appointment && isUpcoming(a.appointment.appointmentDateTime)).length}
                </p>
              </div>
          ...
          <div className="bg-yellow-50 p-4 rounded-lg border border-yellow-200">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold text-yellow-700">{processes.length}</p>
              </div>
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold text-gray-700">
                  {appointments.filter(a => a.appointment && isPast(a.appointment.appointmentDateTime)).length}
                </p>
              </div>
          ...
          <div className="space-y-4">
            <div>
              ...
              <select
                ...
                onChange={(e) => setAppointmentForm(prev => ({ ...prev, processId: e.target.value }))}
                className="w-full p-2 border border-gray-300 rounded-md focus:ring-2 focus:ring-red-500"
              >
                <option value="">Chọn quy trình</option>
                {processes.map(process => (
                  <option key={process.id} value={process.id}>
                    #{process.id} - {process.donor?.fullName} ({process.donor?.email})
                  </option>
                ))}
              </select>
            </div>
            ...
            <InputField
              ...
              onChange={(e) => setAppointmentForm(prev => ({ ...prev, appointmentDateTime: e.target.value }))}
              required
            />
        ...
        >
          <div className="space-y-4">
            <div className="bg-yellow-50 p-3 rounded-lg border border-yellow-200">
              <div className="flex items-center space-x-2">
                <AlertTriangle size={16} className="text-yellow-600" />                <span className="text-sm text-yellow-700">
                  Lịch hẹn hiện tại: {selectedAppointment?.appointment && 
                    formatDateTime(selectedAppointment.appointment.appointmentDateTime)}
                </span>
              </div>
            </div>
            ...
            <InputField
              ...
              onChange={(e) => setRescheduleForm(prev => ({ ...prev, suggestedDateTime: e.target.value }))}
            />
