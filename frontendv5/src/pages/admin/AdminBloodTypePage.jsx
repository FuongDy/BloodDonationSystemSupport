          ...
          {isLoading ? (
            ...
            <div className='bg-white shadow-lg rounded-xl overflow-hidden border border-slate-200'>
              ...
              <div className='border-b border-slate-200'>
                <nav
                  className='-mb-px flex space-x-56 overflow-x-auto px-4'
                  aria-label='Tabs'
                >
                  {Object.keys(groupedBloodTypes)
                    .sort()
                    .map(groupName => (
                      <button
                        key={groupName}
                ...
                {activeTab && groupedBloodTypes[activeTab] ? (
                  <div className='divide-y divide-slate-100'>
                    {groupedBloodTypes[activeTab].map(item => (
                      <div
                        key={item.id}
                        className='flex flex-col md:flex-row items-start md:items-center justify-between p-3 gap-4 hover:bg-slate-50/50 rounded-lg'
                      >
                            ...
                            <>
                              ...
                              <Button
                                onClick={() =>
                                  handleDelete(
                                    item.id,
                                    `${item.bloodGroup} - ${item.componentType}`
                                  )
                                }
                  ...
                  <div className='text-center p-12'>
                    ...
                    <p className='text-slate-500 mt-2'>
                      Không có loại máu nào phù hợp với tìm kiếm của bạn.
                    </p>
                  </div>
                )}
              </div>
