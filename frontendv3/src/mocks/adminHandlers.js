...
let mockBloodTypes = [
    { id: 1, bloodGroup: "A+", componentType: "Whole Blood", description: "A+ Máu toàn phần", shelfLifeDays: 35, storageTempMin: 1, storageTempMax: 6, volumeMl: 450, createdAt: new Date().toISOString() },
    { id: 2, bloodGroup: "O-", componentType: "Red Blood Cells", description: "O- Hồng cầu", shelfLifeDays: 42, storageTempMin: 1, storageTempMax: 6, volumeMl: 250, createdAt: new Date().toISOString() },
    { id: 3, bloodGroup: "B+", componentType: "Plasma", description: "B+ Huyết tương", shelfLifeDays: 365, storageTempMin: -18, storageTempMax: -30, volumeMl: 200, createdAt: new Date().toISOString() },
    { id: 4, bloodGroup: "AB-", componentType: "Platelets", description: "AB- Tiểu cầu", shelfLifeDays: 5, storageTempMin: 20, storageTempMax: 24, volumeMl: 50, createdAt: new Date().toISOString() },
    { id: 5, bloodGroup: "A-", componentType: "Whole Blood", description: "A- Máu toàn phần", shelfLifeDays: 35, storageTempMin: 1, storageTempMax: 6, volumeMl: 450, createdAt: new Date().toISOString() },
];
let nextBloodTypeId = 6;

let mockCompatibilityRules = [
    // Ví dụ rule: O- có thể cho O-, nhận từ O-
    { id: 1, donorBloodType: mockBloodTypes[1], recipientBloodType: mockBloodTypes[1], isCompatible: true, isEmergencyCompatible: true, compatibilityScore: 100, notes: "O- to O- (Self-compatible)", createdAt: new Date().toISOString() },
    // Ví dụ rule: A+ có thể nhận từ A+, A-, O+, O-
    { id: 2, donorBloodType: mockBloodTypes[0], recipientBloodType: mockBloodTypes[0], isCompatible: true, isEmergencyCompatible: true, compatibilityScore: 100, notes: "A+ to A+ (Self-compatible)", createdAt: new Date().toISOString() },
    { id: 3, donorBloodType: mockBloodTypes[4], recipientBloodType: mockBloodTypes[0], isCompatible: true, isEmergencyCompatible: false, compatibilityScore: 80, notes: "A- to A+ (Normal compatible)", createdAt: new Date().toISOString() },
    { id: 4, donorBloodType: mockBloodTypes[1], recipientBloodType: mockBloodTypes[0], isCompatible: true, isEmergencyCompatible: false, compatibilityScore: 75, notes: "O- to A+ (Universal donor for RBC)", createdAt: new Date().toISOString() },
    { id: 5, donorBloodType: mockBloodTypes[1], recipientBloodType: mockBloodTypes[0], isCompatible: true, isEmergencyCompatible: true, compatibilityScore: 90, notes: "O- to A+ (Emergency compatible for RBC)", createdAt: new Date().toISOString() },
    // A+ không cho O-
    { id: 6, donorBloodType: mockBloodTypes[0], recipientBloodType: mockBloodTypes[1], isCompatible: false, isEmergencyCompatible: false, compatibilityScore: 0, notes: "A+ cannot donate to O-", createdAt: new Date().toISOString() },
];
...
export const adminHandlers = [
    ...
    http.get(`${API_URL}/admin/users`, ({ request }) => {
        ...
        return HttpResponse.json({ content, totalElements, totalPages, number: page, size, first: page === 0, last: page >= totalPages - 1 });
    }),
    ...
    http.post(`${API_URL}/blood-types`, async ({ request }) => {
        ...
        const newType = { id: nextBloodTypeId++, ...newTypeReq, createdAt: new Date().toISOString(), updatedAt: new Date().toISOString() };
    ...
    http.put(`${API_URL}/blood-types/:id`, async ({ request, params }) => {
        ...
        // Backend hiện tại chỉ cho phép update description, shelfLifeDays, storageTempMin, storageTempMax, volumeMl
        ...
        mockBloodTypes[typeIndex] = {
            ...
            shelfLifeDays: updatedData.shelfLifeDays !== undefined ? updatedData.shelfLifeDays : mockBloodTypes[typeIndex].shelfLifeDays,
            storageTempMin: updatedData.storageTempMin !== undefined ? updatedData.storageTempMin : mockBloodTypes[typeIndex].storageTempMin,
            storageTempMax: updatedData.storageTempMax !== undefined ? updatedData.storageTempMax : mockBloodTypes[typeIndex].storageTempMax,
            volumeMl: updatedData.volumeMl !== undefined ? updatedData.volumeMl : mockBloodTypes[typeIndex].volumeMl,
            updatedAt: new Date().toISOString(),
        };
    ...
    http.post(`${API_URL}/blood-compatibility`, async ({ request }) => {
        ...
        const newRule = { ...data, id: nextCompatibilityRuleId++, donorBloodType, recipientBloodType, createdAt: new Date().toISOString(), updatedAt: new Date().toISOString() };
    ...
    http.put(`${API_URL}/blood-compatibility/:id`, async ({ request, params }) => {
        ...
        mockCompatibilityRules[ruleIndex] = { ...mockCompatibilityRules[ruleIndex], ...data, donorBloodType, recipientBloodType, updatedAt: new Date().toISOString() };
    ...
    http.get(`${API_URL}/blood-inventory`, ({ request }) => {
        ...
        const inventoryWithDetails = mockBloodInventory.map(item => {
            ...
            return {
                ...item,
                bloodTypeDetails: bloodType ? { bloodGroup: bloodType.bloodGroup, componentType: bloodType.componentType, description: bloodType.description } : null
            };
        });
